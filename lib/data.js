...
// Photos for the rotating 3D ring under the Spotify embeds in "Off duty".
// Travel / food / life shots, all pre-cropped to a uniform 3:4 portrait and
// optimized to webp (see public/photos/offduty/). Order sets where each lands
// on the ring; the carousel itself loops continuously.
export const offDutyPhotos = Array.from(
  { length: 17 },
  (_, i) => `/photos/offduty/photo-${String(i + 1).padStart(2, "0")}.webp`,
);

// Aerial clips for the "Off duty" video row, just under the photo carousel.
// Self-hosted, muted/looped, autoplay gated by on-screen visibility (see
// components/home/OffDutyVideos.js); falls back to native controls only
// under prefers-reduced-motion. `poster` shows before playback starts.
export const offDutyVideos = [
  {
    src: "/videos/offduty/dover-boat.mp4",
    poster: "/videos/offduty/dover-boat-poster.jpg",
    label: "Sailing off Dover at sunset",
  },
  {
    src: "/videos/offduty/lighthouse-1.mp4",
    poster: "/videos/offduty/lighthouse-1-poster.jpg",
    label: "Lighthouse on the cliffs",
  },
  {
    src: "/videos/offduty/lighthouse-2.mp4",
    poster: "/videos/offduty/lighthouse-2-poster.jpg",
    label: "Lighthouse at dusk",
  },
];
